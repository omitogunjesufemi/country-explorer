...
import TextTypwriter from "../components/TextTypewriter";
...
export default function Home() {
    ...
    const regions = [
        ...new Set(data ? data.map((country) => country.region) : []),
    ];

    const filtered = data
        ? data
            .filter((country) => {
                const matchesNSearch =
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-2 py-4 mt-8 mb-8 text-center">
                ...
                <TextTypwriter text={"Explore all the countries in the world"}>
            ...
            <div className="flex gap-3 py-4 mt-8 mb-8 justify-center items-center">
                <input
                    ...
                    className="w-1/2 px-5 py-3 border border-gray-200 rounded-lg focus:border-blue-700 focus:outline-none"
                />

                <AdjustmentsHorizontalIcon
                    className="size-10 shrink-0 text-gray-400 border rounded-md border-gray-200 hover:border-blue-700 hover:scale-105 transition-all duration-200"
                ...
                {region && (
                    <p className="flex items-center gap-1 text-gray-600">
                        <span className="text-sm">Filtering by: {region}</span>
                        <ChevronDoubleLeftIcon
                            className="size-6 text-gray-400 hover:text-red-400 hover:scale-105 transition-all duration-200"
                            onClick={() => setRegion("")}
                        />
                    </p>
                )}
